...
const SchemeVersion = () => {
    ...
    const RenderCategory = props => {
        ...
        return (
            <List className="listScroll" celled divided verticalAlign="middle">
                {listItems}
            </List>
        );
    };
    const RenderSeverity = props => {
        ...
        return (
            <Table singleLine>
                ...
                <Table.Body>{tableItem}</Table.Body>
            </Table>
        );
    };
    ...
    const RenderItem = props => {
        ...
        const tableItem = data.map((x, i) => (
            <Table.Row key={i}>
                ...
                <Table.Cell>
                    <List as="ol">
                        {(Array.isArray(x.info) ? x.info : []).map((y, idx) => (
    ...
    const RenderChecklist = props => {
        ...
        const listItems = data.map((x, i) => (
            <List.Item as="li" key={i}>
                <List.Content>{x.section}</List.Content>
                ...
                <div className="in innerContainer listScroll">
                    <Header as="h3" dividing style={{ lineHeight: "2em" }}>
                        <Button
                            ...
                            color="green"
                            as={Link}
                            to={`${url.split("/version").shift()}`}
                        />
                        Scheme :{" "}
                        {`${detail.cklistName} - v${checklist.version}`}{" "}
                    </Header>
    ...
    } else return <Header as="h3">Loading....</Header>;
};
