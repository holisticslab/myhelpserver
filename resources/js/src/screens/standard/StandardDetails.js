...
const StandardDetails = () => {
    ...
    const RenderItem = props => {
        ...
        const listItems = Object.keys(data).map(ln => (
            <Table.Row key={ln}>
                ...
                <div className="in innerContainer listScroll">
                    <Header as="h3" dividing style={{ lineHeight: "2em" }}>
                        <Button
                            ...
                            color="green"
                            as={Link}
                            to={`${url.split("/details").shift()}`}
                        />
                        Standard : {`${details.code}: ${details.name}`}{" "}
                    </Header>
                    <Input
                        fluid
                        ...
                        onChange={e => {
                            ...
                            Object.keys(stdData).flatMap(
                                pg =>
                                    (newdata[pg] = Object.fromEntries(
                                        Object.entries(stdData[pg]).filter(
                                            ([key, value]) =>
                                                value
                                                    .toLowerCase()
                                                    .indexOf(filter) > -1
